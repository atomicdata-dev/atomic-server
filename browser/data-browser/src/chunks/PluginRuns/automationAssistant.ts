...
export function newAutomationAssistantAsk(
  ...
  return {
    prompt: [
      ...
      'The attached resources are my drive and available integration connections, not a request to use all of them. If one connection is attached, use it as the suggested starting point.',
    ...
    context: [...new Set([drive, ...connections])].map(subject => ({
      type: 'atomic-resource',
      subject,
      id: crypto.randomUUID(),
    })),
