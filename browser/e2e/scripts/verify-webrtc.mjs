...
const result = await build({
  ...
  build: {
    write: false,
    lib: {
      entry: fileURLToPath(new URL('../../lib/src/webrtc-peer.ts', import.meta.url)),
      name: 'AtomicWebrtc', formats: ['iife'],
    },
  },
});
...
for (const [name, launcher] of Object.entries({ chromium, firefox })) {
    ...
    try {
      ...
      for (const page of [a, b]) {
        await page.route('http://localhost/**', route => route.fulfill({ contentType: 'text/html', body: '<!doctype html><title>WebRTC transport acceptance</title>' }));
      ...
      for (const [sender, receiver] of [
        ...
        const received = receiver.evaluate(async length => {
          ...
          return (
            frame.length === length &&
            frame.every((byte, i) => byte === i % 251)
          );
        }, length);
        await sender.evaluate(async length => {
          await window.pipe.send(
