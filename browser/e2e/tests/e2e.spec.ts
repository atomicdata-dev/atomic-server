...
test.describe('data-browser', async () => {
  ...
  test(
    ...
    async ({ page, browser, context }) => {
      ...
      await page.getByLabel('Profile picture (optional)', { exact: true }).setInputFiles({
        name: 'profile.svg',
        mimeType: 'image/svg+xml',
        buffer: Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64"><rect width="64" height="64" fill="#${Math.floor(Math.random() * 0xffffff).toString(16).padStart(6, '0')}"/></svg>`),
      });
      await page.getByRole('dialog').filter({ has: page.getByRole('heading', { name: 'Adjust image' }) }).getByRole('button', { name: 'Save', exact: true }).click();
      await expect(page.getByText('Picture ready to save.', { exact: true })).toBeVisible();
      ...
      const ownerProfile = await page.evaluate(async () => {
        ...
        return { subject, icon: profile.get('https://atomicdata.dev/properties/icon') };
      });
      ...
      expect(sharedProfile).toEqual({ name: 'Drive Owner', icon: ownerProfile.icon });
      const pictureDownload = await page3.evaluate(async icon => {
        const file = await window.store!.fetchResourceFromServer(icon as string);
        if (file.error) throw file.error;
        const url = file.get('https://atomicdata.dev/properties/downloadURL') as string;
      ...
      await page2.goto(`${FRONTEND_URL}${invitePath.pathname}${invitePath.search}`);
      await page2.locator('[data-test="accept-existing"]').click();
      await page2.getByLabel('Full name', { exact: true }).fill('Existing Colleague');
      await page2.getByRole('button', { name: 'Save and continue', exact: true }).click();
      await expect(page2.getByTestId('current-drive-title')).toHaveText(driveTitle);
      await expect(page2.getByRole('heading', { name: 'Agent created!' })).toHaveCount(0);

      // A saved avatar suppresses the profile nudge in both directions.
      await page.goto(`${FRONTEND_URL}${invitePath.pathname}${invitePath.search}`);
      await page.locator('[data-test="accept-existing"]').click();
      await expect(page.getByTestId('current-drive-title')).toHaveText(driveTitle);
      await expect(page.getByRole('heading', { name: 'How your colleagues see you' })).toHaveCount(0);
      ...
      const createAction = page.locator('dialog[open] footer').getByRole('button', { name: 'Create', exact: true });
