...
describe('page-owned HTTP reads', () => {
  it('cancels a pending read when its document is discarded', async () => {
    ...
    const client = new Client(vi.fn((_url, opts) => new Promise((_resolve, reject) => {
    ...
    expect(await request).toMatchObject({ cancelled: true, createdResources: [] });
  });

  it('still reports transport failures on an active page', async () => {
    vi.stubGlobal('window', new EventTarget());
    const client = new Client(vi.fn().mockRejectedValue(new TypeError('Failed to fetch')));
    const result = await client.fetchResourceHTTP('https://example.com/resource');
