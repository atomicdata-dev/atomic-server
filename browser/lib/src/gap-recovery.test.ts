...
describe('a delta that cannot apply triggers a catch-up fetch', () => {
  ...
  it('fetches the missing base for a resource first seen as a live delta', async ({ expect }) => {
    ...
    const outcome = store.applyIncoming({
      subject, loroBytes: orphaned[0], source: 'ws-sub-push',
      commitId: 'did:ad:commit:pending-base',
    });
    expect(outcome).not.toBe('applied');
    expect(asked).toEqual([subject]);
    expect(store.resources.get(subject)?.get(commits.properties.lastCommit)).not.toBe('did:ad:commit:pending-base');
  });
