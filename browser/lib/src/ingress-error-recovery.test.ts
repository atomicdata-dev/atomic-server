...
it.each([false, true])('a valid snapshot clears a stale read error (own echo: %s)', own => {
  ...
  const result = store.applyIncoming({subject, loroBytes:snapshot, commitId, source:'ws-pending-get', replaceLoroDocsFromRemote:true});
...
it('an acknowledged creation clears a pre-genesis not-found lookup', () => {
  ...
  resource.error = new AtomicError('Not found before genesis', ErrorType.NotFound);
  resource.loading = true;
  store.addResource(resource);
  store.applyIncoming({subject:resource.subject, resource, source:'local-acked', commitId:'did:ad:commit:created'});
  expect(resource.error).toBeUndefined();
