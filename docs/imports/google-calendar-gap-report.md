...
|---|---|---|
| Google Calendar API via LocalThought | Implemented, authenticated, browser refresh on folder/table opening | Best current path for stable IDs, series and exception identity. Field coverage still depends on the catalog; new recurrence fields are explicitly preserved. API capabilities exceed what the importer currently exposes. |
