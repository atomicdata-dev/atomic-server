  ...
  records: [
    {
      ...
      name: "Meeting",
      values: {
        start: { dateTime: "2026-09-10T00:30:00+02:00", timeZone: "Europe/Amsterdam" },
...
it("projects timed events without shifting their day and retains provider structures", () => {
  ...
  expect(output.ontology.terms.find((t) => t.shortname === fields.day)?.datatype).toBe(
    Datatype.DATE,
  );
});
it("supports all-day dates and preserves exclusive multi-day ends", () => {
  const input = fixture();
  input.records[0].values = { start: { date: "2026-09-10" }, end: { date: "2026-09-13" } };
...
it("retains cancelled events even when Google omits start", () => {
  ...
  expect(calendarProjection(input).records[0].values[fields.notes]).toContain("Cancelled");
});
...
it("repeated imports reconcile IDs, preserve local fields and don't delete out-of-window events", () => {
  ...
  const properties = Object.fromEntries(
    Object.keys(data.records[0].values).map((k) => [k, `https://example.com/${k}`]),
  );
  const config = {
    platform: data.platform,
    destinations: { event: { table: "did:ad:table", rowClass: "did:ad:event" } },
    properties,
  ...
  expect(run({ ...host, config: { ...config, records: [] } }).intents).toHaveLength(0);
  const changed = calendarProjection(fixture());
  changed.records[0].name = "Updated meeting";
  const result = run({ ...host, config: { ...config, records: changed.records } });
  ...
  expect(run({ ...host, config: { ...config, records: [secondCalendar] } }).intents[0].op).toBe(
    "create",
  );
});
