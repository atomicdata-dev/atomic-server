...
export function calendarProjection(fetched: FetchedPlatform): FetchedPlatform {
  ...
  const event = fetched.ontology.terms.find((t) => t.kind === "class" && t.shortname === "event");
  ...
  if (fetched.ontology.terms.some((t) => terms.some((extra) => extra.shortname === t.shortname)))
    throw new Error("Calendar projection property collides with provider ontology");
  return {
    ...fetched,
    ontology: {
      ...fetched.ontology,
      terms: [
        ...fetched.ontology.terms.map((t) =>
          t === event
            ? { ...t, recommends: [...t.recommends, ...terms.map((extra) => extra.path)] }
            : t,
        ),
        ...terms,
      ],
    ...
    records: fetched.records.map((row) => {
      ...
      if (!cancelled && (typeof date !== "string" || !validDay(date.slice(0, 10))))
        throw new Error(`Calendar event ${row.id} has no valid start date`);
      if (start.date !== undefined && (typeof start.date !== "string" || !validDay(start.date)))
      ...
      if (
        ...
        throw new Error(`Calendar event ${row.id} has no offset-qualified start time`);
      const notes = ["One-way import: edits stay in Atomic"];
      if (cancelled) notes.push("Cancelled in Google; retained in Atomic");
      if (row.values["recurring-event-id"] || row.values.recurringEventId || row.values.recurrence)
      ...
      if (row.values["conference-data"] || row.values.conferenceData)
        notes.push("Conferencing: manage in Google");
      if (end.date || end.dateTime) notes.push("Duration retained in End; shown on start day only");
