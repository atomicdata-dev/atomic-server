...
/// C17: a `did:` resource stamped for drive A must not become a member of
/// drive B's query, even when both drives watch the same filter — neither on
/// the live watched-query path nor on an index rebuild.
#[tokio::test]
#[timeout(120000)]
async fn did_resource_is_only_indexed_in_its_own_drive() {
    use crate::storelike::Query;

    let store = Db::init_temp("query_drive_isolation").await.unwrap();
    let (_alice, drive_a) = store.setup("Alice").await.unwrap();
    let drive_b = store.create_drive("B").await.unwrap();
    let class = "https://example.com/classes/Note";

    let query_in = |drive: &str| {
        let mut q = Query::new_class(class);
        q.sort_by = Some(urls::NAME.to_string());
        q.drive = Some(drive.into());
        q.limit = Some(100);
        q
    };

    // Indexed (rebuild) path: the note exists before either drive's filter is watched.
    let early = store
        .create_resource(class, &drive_a, "early note in A", None)
        .await
        .unwrap();
    let in_b = store.query(&query_in(&drive_b)).await.unwrap();
    assert!(
        !in_b.subjects.iter().any(|s| s.pure_id() == early),
        "a rebuild put drive A's note into drive B's query: {:?}",
        in_b.subjects
    );
    let in_a = store.query(&query_in(&drive_a)).await.unwrap();
    assert!(
        in_a.subjects.iter().any(|s| s.pure_id() == early),
        "drive A's own query must list its note: {:?}",
        in_a.subjects
    );

    // Watched path: both filters are now watched; a new note in A is routed live.
    let late = store
        .create_resource(class, &drive_a, "late note in A", None)
        .await
        .unwrap();
    let in_a = store.query(&query_in(&drive_a)).await.unwrap();
    assert!(
        in_a.subjects.iter().any(|s| s.pure_id() == late),
        "drive A's watched query must pick up its new note: {:?}",
        in_a.subjects
    );
    let in_b = store.query(&query_in(&drive_b)).await.unwrap();
    assert_eq!(
        in_b.count, 0,
        "drive B's watched query must not list drive A's notes: {:?}",
        in_b.subjects
    );
}
