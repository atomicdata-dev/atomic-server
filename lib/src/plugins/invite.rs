use crate::{Resource, Storelike, agents::Agent, errors::AtomicResult, urls};
...
pub fn construct_invite_redirect(
    ...
    for (k, v) in query_params {
        match k.as_ref() {
            "publicKey" | urls::INVITE_PUBKEY=> { pub_key = Some(v.to_string()) },
    ...
    let agent = match (pub_key, invite_agent) {
        ...
        (Some(public_key), None) => {
            ...
            Agent::new_from_public_key(store, &public_key).subject
        },
...
/// Adds the requested rights to the target resource. Overwrites the target resource to include the new rights. Will throw an error if the Agent already has the rights.
pub fn add_rights(agent: &str, target: &str, write: bool, store: &impl Storelike) -> AtomicResult<()> {
    ...
    let mut rights_vector: Vec<String> = match target.get(right) {
        ...
        Ok(val) => {
            ...
            for a in vec {
               if a == agent {
                   return Err("Agent already has these rights".into())
               }
            }
            vec.to_owned()
        },
