...
| [`atomic-lib-runtime.md`](./atomic-lib-runtime.md) | **Partial.** `AtomicNode` is the binding runtime; WASM `ClientDb` and Tauri's native handle bind it. Node startup is separate from the HTTP adapter; the Tauri frontend still needs HTTP/WS. Open: native frontend transport and the other bindings. Local KV FTS landed in [`local-search.md`](./local-search.md). |
