...
var headers = {
  ...
  "User-Agent": "Atomic-Provider-Issues-Pilot",
  "X-Provider-Api-Version": "2022-11-28",
...
function project(issue) {
  if (!Number.isSafeInteger(issue.number) || issue.number <= 0 || typeof issue.title !== "string" || !(issue.body === null || typeof issue.body === "string") || !["open", "closed"].includes(issue.state) || !Array.isArray(issue.labels))
    throw new Error("Provider returned an invalid issue");
  return {
    title: issue.title,
    body: issue.body ?? "",
...
function endpoint(repository) {
  ...
  return `https://provider.example.test/repos/${repository}/issues`;
}
function parse(response) {
  if (response.status < 200 || response.status >= 300)
    throw new Error(
      `Provider returned ${response.status}; no checkpoint was advanced. Resolve access/rate limits before retrying.`
    );
...
async function preview(host, repository) {
  ...
  for (let page = 1; ; page++) {
    ...
    if (!Array.isArray(rows))
      throw new Error("Provider issue page must be an array");
...
async function run(input) {
  ...
  const card = (subject) => {
    ...
    if (number !== void 0 && (!Number.isSafeInteger(number) || Number(number) <= 0))
      throw new Error("Invalid Provider issue number");
    return {
      subject,
  ...
  const issue = (number) => {
    ...
    if (response.status !== 200)
      throw new Error(
        `Provider read failed (${response.status}); no deletion inferred`
      );
  ...
  for (let transitions = 0; transitions < 12; transitions++) {
    ...
    if (cursor.stage === "start") {
      ...
      if (!equal2(project(issue(cursor.number)), desired))
        throw new Error("Provider did not converge; checkpoint paused");
      ...
      const identity = `fixture:${config.repository.toLowerCase()}:issue:${cursor.number}`;
      const set = {
